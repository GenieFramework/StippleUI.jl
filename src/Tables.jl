...
function active_columns(t::T)::Vector{Column} where {T<:DataTable}
  t.opts.columns !== nothing ?
    t.opts.columns :
      [Column(string(name), sortable = true, label = name) for name in TablesInterface.columnnames(t.data)]
end
...
function DataTableOptions(data::T) where T
  ...
  dto.columns = [Column(string(name), sortable = true, label = name) for name in TablesInterface.columnnames(data)]
